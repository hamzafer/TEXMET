...
Features: Retry logic, failure recovery, resume capability.
...
class METDatasetDownloader:
    ...
    def setup_logging(self):
        ...
        self.log_file = self.logs_dir / f"download_{timestamp}.log"
    ...
    def retry_failed_downloads(self):
        ...
        self.logger.info("=== RETRY PHASE COMPLETE ===")
    ...
    def download_all_images(self):
        """Main method to download all images with retry logic"""
    ...
    def save_results_csv(self):
        """Save download results to CSV with detailed information"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def create_summary(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_file = self.output_dir / f"summary_{timestamp}.txt"
        ...
        report = f"""
MET TEXTILES DATASET DOWNLOAD SUMMARY
======================================
...
Total images downloaded: {total_images:,}
  - Primary images: {self.stats['primary_images_downloaded']:,}
  - Additional images: {self.stats['additional_images_downloaded']:,}
Skipped existing files: {self.stats['skipped_existing']:,}
Total download size: {self.stats['total_download_size'] / (1024*1024):.2f} MB
...
def main():
    ...
    if not os.path.exists(args.json_file):
        ...
        print("Please provide a valid path to the dataset JSON file.")
        return
        
    try:
        # Initialize downloader
        downloader = METDatasetDownloader(
        ...
        report_file = downloader.create_summary()
        
        print("\n🎉 DOWNLOAD COMPLETE! 🎉")
        print(f"📊 Detailed Results CSV: {csv_file}")
        print(f"📈 Statistics: {stats_file}")
        print(f"📋 Summary Report: {report_file}")
        print(f"🖼️  Images saved in: {args.output_dir}")
